...
在早期的硬件中，需要进行 4 次 **点过滤（Point Filtering）** 采样，才能得到最终的阴影结果。而在现代的硬件中都集成了此算法，只需要 1 次双线性过滤的特殊采样[SampleCmpLevelZero](https://learn.microsoft.com/en-us/windows/win32/direct3dhlsl/dx-graphics-hlsl-to-samplecmplevelzero)就能得到最终的阴影结果。

下图展示了使用 Bilinear PCF 的阴影结果，阴影贴图的分辨率是 `512x512` ：

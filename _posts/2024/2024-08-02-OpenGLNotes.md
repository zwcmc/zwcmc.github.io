...
- 现代显示器具有非线性亮度响应特性，它的输出在 Gamma 2.2 空间，输入 `0.5` ,最终显示器显示的是 `0.5^2.2`
- 基于物理的光照计算都是在线性空间中的，所以在最终输出到显示器之前，做一次 Gamma 校正： `0.5^(1.0/2.2)` ，也就是转换到 Gamma 0.45（ `1.0/2.2 约等于 0.45` ） 空间，这样最终显示到显示器上的颜色就是线性空间计算的最终结果： `(0.5^0.45)^2.2 = 0.5` （也就是 Gamma 1.0 空间）
- **sRGB 颜色空间对应的是 Gamma 0.45 空间**。艺术家们**基于显示器上看到的颜色创建的和颜色相关的纹理，所以这些纹理都是在 sRGB 空间**。对于这些在 sRGB 空间的纹理，采样后要先将其转换到线性空间，即 `pow(texture(uTexture, uv0).rgb, vec3(2.2))` 。
